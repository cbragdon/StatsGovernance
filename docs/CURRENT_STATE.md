...
The installer now creates the standard Ola-compatible `dbo.CommandLog` table when absent and validates without altering an existing table. A fresh compatibility-level-110 utility database on SQL Server 2025 passed automatic creation, the exact CommandLog contract, an idempotent installer rerun, all governance contracts, and cleanup. A `DBAdmin` rerun retained the existing CommandLog object and all 10 rows.

...
The collector uses `DATABASEPROPERTYEX(DB_NAME(), 'Updateability')` as its writeability gate. Microsoft documents `READ_ONLY` for both [SQL Server availability-group secondaries](https://learn.microsoft.com/en-us/sql/database-engine/availability-groups/windows/configure-read-only-access-on-an-availability-replica-sql-server?view=sql-server-ver17) and [Managed Instance read-only replicas](https://learn.microsoft.com/en-us/azure/azure-sql/database/troubleshoot-common-errors-issues?view=azuresql). This avoids relying on a boxed SQL Server AG function on Managed Instance.
